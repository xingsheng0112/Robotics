...
Eigen::Quaterniond Euler2Quaternion(Eigen::Vector3d euler)
{
    ...
    Q = Eigen::AngleAxisd(euler.z(), Eigen::Vector3d::UnitZ()) *
        ...
        Eigen::AngleAxisd(euler.x(), Eigen::Vector3d::UnitX()); //eular angle ZXY in radians,R to quaternion 
    return Q;
}
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        //std::cout << Q_Total.coeffs().transpose() << std::endl; //(x, y, z, w)

        Eigen::Quaterniond Q_tmp = Eigen::Quaterniond::Identity().slerp(s, Q_Total);
        //std::cout << Q_tmp.coeffs().transpose() << std::endl; //(x, y, z, w)
        ...
        world_point = Q_tmp * world_point ; //重載運算子 qpq-1
        count ++;
        std::cout << "Current point position " << world_point.transpose() << std::endl << std::endl;
