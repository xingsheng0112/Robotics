...
int main(int argc, char **argv)
{
	...
	while (ros::ok()){
		...
		float pos_err_Bx = ( cos(pose.theta) * pos_err.x + sin(pose.theta) * pos_err.y);
		float pos_err_By = (-sin(pose.theta) * pos_err.x + cos(pose.theta) * pos_err.y);
		float goal_theta = atan2(pos_err_By , pos_err_Bx);
		
		printf("goal_theta : %f \t" ,goal_theta);
		
		float vel = sqrt(pow(pos_err_Bx, 2) + pow(pos_err_By, 2));
		
		vel_msg.linear.x = vel;
		vel_msg.angular.z = goal_theta;
